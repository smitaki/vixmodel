...
# New from paper: VIX premium range (4-5 pts overestimation)
VIX_PREMIUM = 4.5

# ----------------------------------------------------
# DATA & FEATURES - ADDED SKEWNESS/KURTOSIS + PAPER MATH
# ----------------------------------------------------
...
def load_and_process_data():
    ...
    # NEW: Rolling skewness and kurtosis (from paper's tail cumulants)
    df["skew_30d"] = df["daily_return"].rolling(30).skew()
    df["kurt_30d"] = df["daily_return"].rolling(30).kurt()
    
    # NEW: Expected VIX (recent vol + reversion adj + premium, per paper)
    df["expected_vix"] = df["rolling_vol_30d"] + (df["CLOSE"].rolling(30).mean() - df["CLOSE"]) * 0.5 + VIX_PREMIUM  # Simplified reversion adj
    
    # NEW: VCR (VIX Change Ratio, paper's error metric)
    df["vcr"] = (df["CLOSE"] - df["expected_vix"]) / df["expected_vix"]
    
    # NEW: Spike premium (overestimation, high = buy vol)
    ...
    # Signals with analogs - MATURED WITH PAPER MATH (SKEW/KURT/VCR FILTERS)
    df["signal"] = "HOLD"
    df["signal_reason"] = ""
    ...
    if len(df) > 1:
        ...
        # Historical analogs - ADDED SKEW/KURT FILTER FOR TAIL MATURITY
        hist = df.iloc[:-1]
        ...
        skew_mask = abs((hist["skew_30d"] - latest["skew_30d"]) / latest["skew_30d"]) <= 0.2 if not pd.isna(latest["skew_30d"]) else True
        kurt_mask = abs((hist["kurt_30d"] - latest["kurt_30d"]) / latest["kurt_30d"]) <= 0.2 if not pd.isna(latest["kurt_30d"]) else True
        analogs = hist[level_mask & vol_mask & regime_mask & skew_mask & kurt_mask]
        prob_spike = (analogs["fwd_1d_vix_chg"] >= DAILY_SPIKE_THRESH).mean() if len(analogs) > 0 else np.nan
        
        if (latest["regime"] == "calm") and (latest["CLOSE"] < mean_vix * 0.95) and (latest["vcr"] > 0.02):  # VCR filter per paper
            signal = "STRONG_BUY_VIX_CALLS" if prob_spike > 0.5 else "BUY_VIX_CALLS"
            reason = f"Calm regime + low level + high VCR ({latest['vcr']:.2%}). Analogs: {len(analogs)}, prob spike: {prob_spike:.1%}" if not np.isnan(prob_spike) else "Strong complacency"
            df.at[df.index[-1], "signal"] = signal
            df.at[df.index[-1], "signal_reason"] = reason + f" | Spike Premium: {latest['spike_premium']:.1f} | ~{est_days} days to spike"
        
        elif (latest["regime"] in ["calm", "normal"]) and (latest["rolling_vol_30d"] < df["rolling_vol_30d"].quantile(0.4)) and (latest["kurt_30d"] > 1):  # Fat tails filter
            signal = "STRONG_BUY_VIX_CALLS" if prob_spike > 0.5 else "BUY_VIX_CALLS"
            reason = f"Low vol setup + fat tails (kurt {latest['kurt_30d']:.1f}). Analogs: {len(analogs)}, prob spike: {prob_spike:.1%}" if not np.isnan(prob_spike) else "Complacency building"
            df.at[df.index[-1], "signal"] = signal
            df.at[df.index[-1], "signal_reason"] = reason + f" | Spike Premium: {latest['spike_premium']:.1f} | ~{est_days} days"
        
        elif (latest["regime"] == "stressed") and (latest["spike_cluster"] in ["medium", "large"]) and (latest["vcr"] < 0):
            signal = "STRONG_SELL_VIX" if prob_spike < 0.3 else "SELL_VIX"
            reason = f"Large spike + negative VCR ({latest['vcr']:.2%}). Analogs: {len(analogs)}, prob spike: {prob_spike:.1%}" if not np.isnan(prob_spike) else "Expect fade"
            df.at[df.index[-1], "signal"] = signal
            df.at[df.index[-1], "signal_reason"] = reason + f" | Spike Premium: {latest['spike_premium']:.1f}"
